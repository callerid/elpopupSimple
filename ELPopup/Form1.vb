...
#End Region
    ...
    Public Sub NetworkEnd()
        GeneralMessage("EL Popup will suspend operation while EL Config is running.", "EL Popup Suspended")
    ...
    Private Sub DataHandler(ByVal sDataText As String)
        ...
        If record.IsTimeAutoGenerated Then
            ...
            'Apparently, users find it "annoying" when balloon popups happen every call (in some situations).
            'If Growl.IsGrowlRunning Then
            'growl.Notify(New Growl.Connector.Notification("EL Popup", "GENERIC", "ID", "Resetting Caller ID Time", "The Whozz Calling's clock is now set to match this computer's clock."))
            'Else
            'TrayIcon.ShowBalloonTip(1, "Resetting Caller ID Time", "The Whozz Calling's clock is now set to match this computer's clock.", ToolTipIcon.Info)
            'End If
    ...
    Private Sub DataReceivedEventHandler() Handles UdpReceiver.DataReceived
        Dim sReceivedText As String = UdpReceiver.sReceivedMessage
